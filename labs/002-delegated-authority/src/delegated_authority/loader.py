"""Load Lab 002 educational policy and example data."""
...
class Lab002ConfigError(ValueError):
    """Raised when Lab 002 data cannot be loaded safely."""
...
def _read_yaml(path: Path) -> Any:
    try:
        ...
        raise Lab002ConfigError("Invalid YAML: {0}".format(path.name)) from exc


def _index_by_id(items: Any, key: str = "id") -> Dict[str, Dict[str, Any]]:
    if not isinstance(items, list):
        raise Lab002ConfigError("Expected a list of mappings.")
    out: Dict[str, Dict[str, Any]] = {}
    for raw in items:
        if not isinstance(raw, dict) or key not in raw:
            raise Lab002ConfigError("Each item must include '{0}'.".format(key))
        item_id = str(raw[key])
        if item_id in out:
            raise Lab002ConfigError("Duplicate id '{0}'.".format(item_id))
        out[item_id] = raw
    return out
...
def load_bundle(
    ...
    for name in required:
        if not (data / name).is_file():
            raise Lab002ConfigError("Missing example file: {0}".format(name))
    for name in (
        "delegation-policy.yaml",
        "posture-policy.yaml",
        "fallback-policy.yaml",
    ):
        if not (policies_path / name).is_file():
            raise Lab002ConfigError("Missing policy file: {0}".format(name))
    ...
    if not isinstance(scenarios_raw, list):
        raise Lab002ConfigError("scenarios must be a list.")
    scenarios: Dict[str, Dict[str, Any]] = {}
    ...
    policies = {
        "delegation": _read_yaml(policies_path / "delegation-policy.yaml"),
        "posture": _read_yaml(policies_path / "posture-policy.yaml"),
        "fallback": _read_yaml(policies_path / "fallback-policy.yaml"),
    }
    return Lab002Bundle(
        agents=agents,
        profiles=profiles,
